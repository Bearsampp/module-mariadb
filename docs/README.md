# MariaDB Module Documentation

This directory contains documentation for the Bearsampp MariaDB module.

## Available Documentation

### [CI/CD Testing](CI-CD-TESTING.md)

Comprehensive guide to the automated testing workflow for MariaDB builds:
- **Smart Version Detection**: 2-tier cascading detection system
  - Primary: `/bin` directory changes detection
  - Fallback: PR title version extraction
  - Final Fallback: Latest 5 versions
- Workflow triggers and configuration
- Detailed test phase descriptions
- Test results and artifacts
- Troubleshooting guide
- Maintenance instructions

## Quick Links

- **Workflow File**: [`.github/workflows/mariadb-test.yml`](../.github/workflows/mariadb-test.yml)
- **Releases**: [`releases.properties`](../releases.properties)
- **Main Repository**: [Bearsampp MariaDB Module](https://github.com/Bearsampp/module-mariadb)

## Contributing

When adding new features or making changes to the MariaDB module:

1. Update relevant documentation in this folder
2. Ensure CI/CD tests pass on your pull request
3. Review test results in the automated PR comment
4. Update this README if adding new documentation files

## Support

For issues or questions:
- Open an issue on [GitHub](https://github.com/Bearsampp/module-mariadb/issues)
- Check existing documentation for troubleshooting steps
- Review CI/CD test logs for build-related issues
